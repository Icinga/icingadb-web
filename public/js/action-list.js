(function (Icinga) {
    ...
    try {
        ...
        console.warn('Unable to provide action list feature. Libraries not available:', e);
        return;
    }
    ...
    class ActionListBehavior extends Icinga.EventListener {
        ...
        onBeforeRender(event, content, action, autorefresh, scripted) {
            ...
            let lists = event.target.querySelectorAll('[data-interactable-action-list]');
        ...
        onRendered(event, autorefresh, scripted) {
            ...
            if (event.target.id === 'col2') {
                console.log(event, autorefresh, scripted, detailUrl);
            }
            ...
            event.currentTarget.querySelectorAll('[data-interactable-action-list]').forEach(list => {
                let actionList = _this._actionLists.get(list);
        ...
        {
            let _this = event.data.self;
            let lists = document.querySelectorAll('[data-interactable-action-list]');
            lists.forEach((list) => {
                let actionList = _this._actionLists.get(list);
                if (actionList) {
                    actionList.load();
